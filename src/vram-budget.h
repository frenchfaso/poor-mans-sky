...
static size_t vramEstimate(void) {
 ...
 bytes+=voxelGPUBytes+hybridGPUBytes;
 /* Explicit cloud allocation upper bound; no driver query in the render loop. */
 bytes+=(cloudTex?256u*256u*4u:0)+(cloudVbo?96u*3u*18u*sizeof(CloudVertex):0);
...
static int vramReserve(size_t incoming) {
 ...
 /* Cached curtains are rebuildable and must yield to scene/actor allocations. */
 while(vramEstimate()+incoming>VRAM_BUDGET && hybridEvict()){}
 SDL_LockMutex(mutex);
