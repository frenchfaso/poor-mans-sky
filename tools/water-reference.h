...
/* Frozen pre-optimization algorithm: regression oracle, test-only RAM. */
static Vertex referenceWaterGrids[SLOTS][WATER_NV];
static void referenceWaterSeams(void) {
 ...
 for(int i=0;i<selectedCount;i++) {
  ...
  for(int j=0;j<WATER_NV;j++) {
   ...
   referenceWaterGrids[n->slot][j]=q;
  }
 }
 for(int i=0;i<selectedCount;i++) {
  ...
  for(int r=0;r<neighbors->count;r++) {
   ...
   for(int k=run->first;k<=run->last;k++) {
    Vertex *out=&referenceWaterGrids[n->slot][edgeVertex(run->edge,k)];double d[3],u,v,scale=1.0/(1u<<o->level);
    ...
    Vertex sample=sampleMesh(referenceWaterGrids[o->slot],u,v,o->meshLevel);
...
static int waterMatchesReference(void) {
 int visible=0;
 for(int i=0;i<selectedCount;i++)
  visible|=nodes[selected[i]].visible && nodes[selected[i]].minHeight<0;
 if(!visible)return 1;
 referenceWaterSeams();
 for(int i=0;i<selectedCount;i++) {
  Node *n=&nodes[selected[i]];
  if(n->slot>=0 && n->vertices && n->minHeight<0 &&
     memcmp(waterGrids[n->slot],referenceWaterGrids[n->slot],sizeof(waterGrids[0])))return 0;
 }
 return 1;
}
