...
assert len(data)==16+256*256*4, 'wrong payload size'
assert data[:8]==b'PMSCLOUD' and struct.unpack('<II',data[8:16])==(256,256), 'wrong header'
tiles=[]
for variant in range(12):
    alpha=[]
    for y in range(64):
        for x in range(64):
            i=16+(((variant//4)*64+y)*256+(variant%4)*64+x)*4
            r,g,b,a=data[i:i+4]
...
assert len(set(tiles))==12, 'cloud variants must differ'
print('PASS cloud atlas: 256 KiB RGBA, twelve distinct neutral tiles, transparent borders and octagon padding')
